...
const { prefix, openPages, name: website } = config
...
const App = ({
  ...
  const freeMenu = ['/account/userInfo', '/settings']; // 放行的页面
  const hasPermission = freeMenu.indexOf(pathname) !== -1 || (current.length ? permissions.visit.includes(current[0].id) : false)
  ...
  return (
    <div>
      ...
      <Layout className={classnames({ [styles.dark]: darkTheme, [styles.light]: !darkTheme })}>
        ...
        <Layout style={{ height: '100vh', overflow: 'hidden scroll' }} id="mainContainer">
