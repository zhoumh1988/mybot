...
const BASE_PATH = '/api/stat/roomword';

const RoomWordService = function (app) {
    /**
     * 用户字数列表接口
     */
    app.post(`${BASE_PATH}/list`, (request, response) => {
        let { roomid, start, end } = request.body;
        let $sql = 'SELECT date,type,info FROM stat_room_word WHERE 1=1 ';
        let params = [];
        ...
        _pool_connection_format($sql, params, rs => {
            if (rs !== false) {
                ...
                resObj = Utils.completDate({ start, end, resObj, attrs: ['1', '5', '10', '20'] })
                let res = [];
    ...
    */
    app.post(`${BASE_PATH}/pie`, (request, response) => {
        ...
        let $sql = 'SELECT date,type,info FROM stat_user_word WHERE 1=1 ';
        let { roomid, start, end } = request.body;
        let params = [];
        if (typeof roomid == 'string' && Utils.isNotEmpty(roomid)) {
            params.push(roomid);
...
module.exports = RoomWordService;
