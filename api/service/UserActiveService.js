...
const BASE_PATH = '/api/stat/useractive';

const UserActiveService = function (app) {
    /**
     * 用户活跃列表接口
     */
    app.post(`${BASE_PATH}/list`, (request, response) => {
        let { roomid, start, end } = request.body;
        let $sql = 'SELECT date,type,info FROM stat_user_active WHERE 1=1 ';
        let params = [];
    ...
    app.post(`${BASE_PATH}/listbyactive`, (request, response) => {
        ...
        let params = [];
        let $sql = 'SELECT a.date,b.topic,a.info FROM stat_user_active a left join (select roomid,topic from room order by id asc limit 10 ) b on a.roomid=b.roomid where 1=1  ';
        if (typeof start == 'string' && Utils.isNotEmpty(start)) {
            let _time = Utils.moment(start);
            if (!_time.isValid()) {
                let dto = new DTO(400, `开始时间格式错误：${start}，正确格式：YYYY-MM-DD`);
    ...
    */
    app.post(`${BASE_PATH}/listbydate`, (request, response) => {
        ...
        let params = [];
        let $sql = 'SELECT a.type,b.topic,a.info FROM stat_user_active a left join (select roomid,topic from room order by id asc limit 10 ) b on a.roomid=b.roomid where 1=1  ';
